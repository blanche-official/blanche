...
from blanche.browser.node_type import (
    HtmlSelector,
    NodeAttributesPre,
    NodeCategory,
    NodeRole,
    BlancheAttributesPost,
    BlancheNode,
)
...
def test_blanche_node():
    ...
    # Test string role
    string_role_node = BlancheNode(id="str1", role="custom_role", text="Custom")
    assert string_role_node.get_role_str() == "custom_role"
    assert string_role_node.is_interaction() is False


def test_blanche_node_flatten():
    # Create a nested structure
    button1 = BlancheNode(id="btn1", role=NodeRole.BUTTON, text="Button 1")
    button2 = BlancheNode(id="btn2", role=NodeRole.BUTTON, text="Button 2")
    text_node = BlancheNode(id="txt1", role=NodeRole.TEXT, text="Some text")

    group = BlancheNode(id="group1", role=NodeRole.GROUP, text="Group", children=[button1, text_node, button2])
...
def test_node_attributes():
    ...
    # Test BlancheAttributesPost
    selector = HtmlSelector("test", "test", "test")
    post_attrs = BlancheAttributesPost(selectors=selector, editable=True, input_type="text", text="Sample text")
...
def nested_graph() -> BlancheNode:
    return BlancheNode(
        ...
        text="Root",
        children=[
            BlancheNode(id="B1", role=NodeRole.BUTTON, text="Button 1"),
            BlancheNode(id="B2", role=NodeRole.BUTTON, text="Button 2"),
            BlancheNode(id=None, role=NodeRole.TEXT, text="Some text"),
            BlancheNode(
                id=None,
                role=NodeRole.GROUP,
                text="Group",
                children=[
                    BlancheNode(id="B3", role=NodeRole.BUTTON, text="Button 3"),
                    BlancheNode(id=None, role=NodeRole.TEXT, text="Some other text"),
                ],
            ),
            BlancheNode(id="B4", role=NodeRole.BUTTON, text="Button 4"),
            BlancheNode(id=None, role=NodeRole.TEXT, text="Some text 3"),
            BlancheNode(
                id=None,
                role=NodeRole.GROUP,
                text="Group 2",
                children=[
                    BlancheNode(id="L1", role=NodeRole.LINK, text="Link 1"),
                    BlancheNode(id=None, role=NodeRole.TEXT, text="Some text 4"),
                ],
            ),
        ],
    )
...
def test_subtree_exclude_all_nodes(nested_graph: BlancheNode):
    def exclude_all_nodes(node: BlancheNode) -> bool:
        return False
...
def test_subtree_keep_all_nodes(nested_graph: BlancheNode):
    def keep_all_nodes(node: BlancheNode) -> bool:
        return True
...
def test_subtree_keep_one_node(nested_graph: BlancheNode):
    def exclude_some_nodes(node: BlancheNode) -> bool:
...
def test_subtree_keep_one_node_2(nested_graph: BlancheNode):
    def exclude_all_except_B2(node: BlancheNode) -> bool:
...
def test_subtree_keep_some_nodes(nested_graph: BlancheNode):
    def keep_some_nodes(node: BlancheNode) -> bool:
