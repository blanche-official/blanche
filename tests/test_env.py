...
@pytest.fixture
async def env_generator(mock_llm_service: MockLLMService) -> AsyncGenerator[BlancheEnv, None]:
    """Create a BlancheEnv instance with mock browser and LLM"""
    browser = MockBrowserDriver()
    async with BlancheEnv(browser=browser, llmserve=mock_llm_service) as env:
        yield env


@pytest.fixture
async def aenv(env_generator: AsyncGenerator[BlancheEnv, None]) -> BlancheEnv:
    """Helper fixture that returns the BlancheEnv instance directly"""
    return await anext(env_generator)


@pytest.mark.asyncio
async def test_context_property_before_observation(aenv: Awaitable[BlancheEnv]) -> None:
...
@pytest.mark.asyncio
async def test_context_property_after_observation(aenv: Awaitable[BlancheEnv]) -> None:
...
@pytest.mark.asyncio
async def test_trajectory_empty_before_observation(aenv: Awaitable[BlancheEnv]) -> None:
...
@pytest.mark.asyncio
async def test_valid_observation_after_observation(aenv: Awaitable[BlancheEnv]) -> None:
...
@pytest.mark.asyncio
async def test_valid_observation_after_step(aenv: Awaitable[BlancheEnv]) -> None:
...
@pytest.mark.asyncio
async def test_valid_observation_after_reset(aenv: Awaitable[BlancheEnv]) -> None:
