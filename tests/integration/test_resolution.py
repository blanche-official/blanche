...
from blanche.actions.base import Action, ActionParameter, ActionParameterValue
...
async def _test_action_node_resolution_pipe(url: str) -> None:
    errors: list[str] = []
    total_count = 0
    async with BlancheEnv(headless=True) as env:
...
async def test_blanche() -> None:
    """Test resolution pipe with Blanche homepage"""
    await _test_action_node_resolution_pipe("https://blanche.cc")
