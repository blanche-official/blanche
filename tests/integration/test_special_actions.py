...
from blanche.actions.base import SpecialAction, SpecialActionId
...
async def test_special_actions_execution(llm_service: MockLLMService):
    ...
    async with BlancheEnv(headless=True, llmserve=llm_service, screenshot=False) as env:
        # Test S1: Go to URL
        obs = await env.execute(SpecialActionId.GOTO, {"url": "https://blanche.cc/"})
        assert obs.clean_url == "blanche.cc"
...
async def test_special_action_validation(llm_service: MockLLMService):
    ...
    async with BlancheEnv(headless=True, llmserve=llm_service) as env:
        _ = await env.goto("https://blanche.cc/")
