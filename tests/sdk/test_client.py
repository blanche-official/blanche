...
from blanche.actions.space import SpaceCategory
from blanche.browser.observation import Observation
from blanche.sdk.client import BlancheClient
from blanche.sdk.types import (
...
@pytest.fixture
def client(api_key: str, server_url: str) -> BlancheClient:
    return BlancheClient(
...
def test_client_initialization_with_env_vars() -> None:
    client = BlancheClient(server_url="http://my-server.com", api_key="test-api-key")
...
def test_client_initialization_with_params() -> None:
    client = BlancheClient(api_key="custom-api-key", server_url="http://custom-url.com")
...
def test_client_initialization_without_api_key() -> None:
    with patch.dict(os.environ, clear=True):
        with pytest.raises(ValueError, match="BLANCHE_API_KEY needs to be provide"):
            _ = BlancheClient()
...
def _start_session(mock_post: MagicMock, client: BlancheClient, session_id: str) -> SessionResponse:
...
@patch("requests.post")
def test_start_session(mock_post: MagicMock, client: BlancheClient, api_key: str, session_id: str) -> None:
...
@patch("requests.post")
def test_close_session(mock_post: MagicMock, client: BlancheClient, api_key: str, session_id: str) -> None:
...
@patch("requests.post")
def test_scrape(mock_post: MagicMock, client: BlancheClient, api_key: str, session_id: str) -> None:
    mock_response = {
        "metadata": {
            "title": "Test Page",
...
@patch("requests.post")
def test_scrape_without_url_or_session_id(mock_post: MagicMock, client: BlancheClient) -> None:
...
@patch("requests.post")
def test_observe(mock_post: MagicMock, client: BlancheClient, api_key: str, start_session: bool, session_id: str) -> None:
    if start_session:
...
@pytest.mark.parametrize("start_session", [True, False])
@patch("requests.post")
def test_step(mock_post: MagicMock, client: BlancheClient, api_key: str, start_session: bool, session_id: str) -> None:
    if start_session:
        _ = _start_session(mock_post, client, session_id)
    mock_response = {
        "session": session_response_dict(session_id),
...
def test_format_observe_response(client: BlancheClient, session_id: str) -> None:
    response_dict = {
        "status": 200,
        "session": session_response_dict(session_id),
        "metadata": {
