...
class A11yNode(TypedDict, total=False):
    ...
    markdown: str
    # added by the blanche processing
    id: str
...
class BlancheNode:
    ...
    children: list["BlancheNode"] = field(default_factory=list)
    attributes_pre: NodeAttributesPre = field(default_factory=NodeAttributesPre.empty)
    attributes_post: BlancheAttributesPost | None = None
    ...
    @staticmethod
    def from_a11y_node(node: A11yNode, path: str = "") -> "BlancheNode":
        node_path = ":".join([path, node["role"], node["name"]])
        children = [BlancheNode.from_a11y_node(child, node_path) for child in node.get("children", [])]
        return BlancheNode(
            id=node.get("id"),
    ...
    def find(self, id: str) -> "BlancheNode | None":
        if self.id == id:
            return self
        for child in self.children:
            found = child.find(id)
    ...
    def flatten(self, only_interaction: bool = False) -> list["BlancheNode"]:
        base: list["BlancheNode"] = [] if only_interaction and not self.is_interaction() else [self]
    ...
    def image_nodes(self) -> list["BlancheNode"]:
        return [node for node in self.flatten() if node.is_image()]

    def subtree_filter(self, ft: Callable[["BlancheNode"], bool]) -> "BlancheNode | None":
        def inner(node: BlancheNode) -> BlancheNode | None:
            children = node.children
            if not ft(node):
                return None

            filtered_children: list[BlancheNode] = []
    ...
    def subtree_without(self, roles: set[str]) -> "BlancheNode":

        def only_roles(node: BlancheNode) -> bool:
...
class InteractionNode(BlancheNode):
    id: str  # type: ignore
