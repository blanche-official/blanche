...
from blanche.data.space import DataSpace
...
class BlancheClient:
    """
    Client for the Blanche API.

    Note: this client is only able to handle one session at a time.
    If you need to handle multiple sessions, you need to create a new client for each session.
    """

    DEFAULT_SERVER_URL: ClassVar[str] = "https://api.blanche.cc"
    ...
    def __init__(
        ...
        self.token = api_key or os.getenv("BLANCHE_API_KEY")
        if self.token is None:
            raise ValueError("BLANCHE_API_KEY needs to be provided")
    ...
    def start(self, **data: Unpack[SessionRequestDict]) -> SessionResponse:
        if self._base_session_request is not None and self._base_session_request.session_id is not None:
            logger.warning("One Blanche session already exists. Closing it before starting a new one...")
            _ = self.close()
    ...
    def __enter__(self) -> "BlancheClient":
        _ = self.start()
        return self
