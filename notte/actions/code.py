from blanche.actions.base import ActionParameterValue, ExecutableAction
from blanche.browser.context import Context
from blanche.browser.node_type import HtmlSelector, BlancheNode
...
def get_action_from_node(node: BlancheNode) -> str:
    if node.id is None:
...
def compute_playwright_code(
    ...
    node = action.node
    if node is None:
        raise ValueError(f"Target Blanche node for action {action.id} must be provided")
    if node.attributes_post is None:
